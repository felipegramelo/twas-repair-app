...
export default function ChangePasswordScreen() {
  ...
  const handleChangePassword = async () => {
    ...
    if (newPassword.length < 6) {
      if (Platform.OS === 'web') window.alert('A nova senha deve ter no mínimo 6 caracteres');
      else Alert.alert('Erro', 'A nova senha deve ter no mínimo 6 caracteres');
      return;
    }
    if (newPassword !== confirmPassword) {
      if (Platform.OS === 'web') window.alert('As senhas não coincidem');
