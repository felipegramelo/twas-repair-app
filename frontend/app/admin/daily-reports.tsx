...
export default function DailyReportsScreen() {
  ...
  const loadReports = async () => {
    try {
      ...
      setReports(all.filter(r => r.report_type === 'daily'));
    } catch (error) {
      console.error('Erro ao carregar relatórios:', error);
      Alert.alert('Erro', 'Erro ao carregar relatórios diários');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = async (report: Report) => {
    try {
      if (Platform.OS === 'web') {
        ...
        link.href = url;
        link.download = `relatorio_diario_${report.service_order_number}.pdf`;
  ...
  // Count daily entries in report sections
  const getDailyEntryCount = (report: Report) => {
    return report.sections.filter(s => s.section_type.startsWith('daily_entry_')).length;
  };
    ...
    <View style={styles.card} data-testid={`daily-report-card-${item.id}`}>
      <View style={styles.topRow}>
        ...
        <View style={styles.actions}>
          <TouchableOpacity onPress={() => handleOpenPDF(item)} style={styles.actionBtn} data-testid={`dr-pdf-${item.id}`}>
            <Ionicons name="document-text-outline" size={20} color="#1a237e" />
          </TouchableOpacity>
          <TouchableOpacity onPress={() => handleDownloadPDF(item)} style={styles.actionBtn} data-testid={`dr-download-${item.id}`}>
      ...
      <View style={styles.cardInfo}>
        ...
        {getDailyEntryCount(item) > 0 && (
          <Text style={styles.entryCount}>{getDailyEntryCount(item)} dia(s) registrado(s)</Text>
        )}
...
const styles = StyleSheet.create({
  ...
  entryCount: { fontSize: 12, color: '#1a237e', marginTop: 4, fontWeight: '500' },
