...
- **2026-06** — Botão de atualização: endpoint `GET /api/version` (constante `APP_VERSION` em `server.py`) + `frontend/constants/appVersion.ts` + componente `UpdateChecker` montado no `_layout.tsx`. No web, verifica a cada 5 min e no focus da janela; se a versão do servidor difere, mostra banner "Nova versão disponível — Atualizar" (limpa caches e recarrega). REGRA: ao lançar release, bump nas DUAS constantes (backend e frontend).
