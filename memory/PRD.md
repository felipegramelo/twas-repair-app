...
- **2026-06** — Atualização iOS preparada: `app.json` bumped para v1.0.20 + `runtimeVersion: {policy: "appVersion"}` adicionado. Descoberto que builds anteriores NUNCA tiveram EAS Update configurado (sem `updates.url`/`projectId` no histórico git) → OTA não funciona nos builds instalados. Guia entregue ao usuário: no Mac rodar `eas update:configure` → `eas build --platform ios --profile production` → `eas submit --platform ios --latest`. Após esse build, futuras atualizações JS via `eas update --branch production`.
