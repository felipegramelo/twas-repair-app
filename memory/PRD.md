...
- **2026-08** — ✅ Refinamento do timesheet PDF: caixa de Observações agora tem altura dinâmica (preenche o espaço restante da página, assinatura rente ao rodapé, sem vão em branco), mantendo 1 página garantida. Implementado via page_start_idx + soma de wrap() dos flowables em routes/timesheets.py. Validado visualmente com dados reais do usuário (10 lançamentos + obs de 7 linhas). Redeploy disparado com autorização.
- **2026-08** — ✅ Timesheet PDF em 2 páginas corrigido: layout estava a 23,86cm dos 24,00cm disponíveis; compactado padding da tabela de horas (0.15→0.08cm), obs box (4.0→3.4cm) e info table (bottom 0.2→0.1cm) em routes/timesheets.py. Reproduzido e validado com timesheet sintético de 10 lançamentos: agora 1 página, layout íntegro (render visual conferido). PENDENTE: redeploy para produção (junto com normalização do nº OS).
- **2026-08** — ✅ Nº da OS travado no formato padrão (sem espaços): (1) frontend service-orders.tsx remove espaços no onChangeText; (2) backend normalize_os_number em create/update de OS; (3) proposals.py generate_os_number_from_proposal corrigido (era `f"{seq:02d} - {num}"` COM espaços — origem real do bug!) → agora `{seq:02d}-{num}` normalizado; (4) migração idempotente no startup do server.py normaliza os_number em service_orders/timesheets/reports/projects/propostas (rodou no preview: 0 restantes; rodará em produção no próximo deploy). Testado e2e via curl: "99 - 2699 - 01" → salvo "99-2699-01". ATENÇÃO: deploy anterior foi disparado ANTES dessas mudanças — precisa de novo redeploy para produção receber isto.
- **2026-08** — ✅ Normalização do nº da OS nos nomes de pasta OneDrive: `build_folder_name` (backend/services/onedrive.py) agora remove espaços ao redor de hífens no os_number (`38 - 2607 - 73` → `38-2607-73`), evitando pastas duplicadas quando a OS foi digitada com formato diferente (DB tem formatos mistos). Vale para Reports/Timesheets/Projects. Testado via python (ambos formatos geram nome idêntico). Redeploy do backend disparado com autorização do usuário. Usuário orientado a renomear no OneDrive as pastas antigas com número "espaçado" (ex: OS 37) e mesclar a dup da OS 38.
- **2026-08 (via Make.com, sem código)** — ✅ BUG pastas duplicadas OneDrive (Timesheets) RESOLVIDO: cenário "TWAS Timesheets → OneDrive" criava pasta numerada a cada PDF (~20 dups) porque a fórmula do Tools/Set variable `pastas` tinha referências quebradas (avaliava vazio) → filtro "Pasta existe" nunca passava → Create a Folder (conflito=rename) numerava. Fórmula reconstruída com o usuário: `map(get(5.Body; value); id; name; 2.folder)`. Teste confirmado: run com 4 operações, Create a Folder bloqueado pelo filtro, Upload via caminho "Pasta existe". LIÇÃO: ao copiar módulos entre cenários Make, pills quebram silenciosamente; diagnosticar via History → Module inspector → Output. Usuário orientado a limpar pastas numeradas e vigiar o cenário de Reports (mesma correção se ocorrer).
- **2026-06/07** — ✅ Atualização iOS CONCLUÍDA: usuário guiado passo a passo no MacBook (pasta ~/Desktop/twas-repair-app). Fluxo: Save to GitHub → git reset --hard origin/main → yarn install → eas update:configure (projectId 67451cae-7886-49ff-a69b-46c310f079ae, conta Expo felipegmelo) → aceitou novo Apple Developer License Agreement (bloqueava registro do bundle) → eas build ios production (v1.0.20, build 24, appVersionSource=remote) → eas submit OK (ASC App ID 6761557568). app.json/eas.json da Emergent sincronizados com updates.url + projectId + channels (production/preview/development). FUTURAS atualizações JS: apenas `eas update --branch production` (OTA, sem Apple). PENDENTE: build Android novo (fix OneDrive só chega com novo build) e possível "Missing Compliance" no TestFlight (responder "No").
- **2026-06** — BUG Android/OneDrive corrigido (verificado por testing_agent, 10/10 testes): downloads nativos não enviavam `?download=1`, então o webhook do OneDrive nunca disparava no Android/iOS. Corrigido em admin/timesheets.tsx (handleDownloadPDF), supervisor/index.tsx (timesheet + report download) e supervisor/edit-report.tsx (handleSharePDF, agora com downloadPDF(id, true)). Preview (handleOpenPDF) segue SEM download=1 (não deve subir ao OneDrive). ATENÇÃO: fix chega no Android só via novo build/EAS Update.
- **2026-06** — Deploy Emergent concluído (shift-docs-staging.emergent.host). Bloqueadores corrigidos: .gitignore sem .env, expo --tunnel no supervisor, config.ts detecta hosts Emergent (*.emergentagent.com/*.emergent.host → env URL; Vercel/nativo → Railway).
